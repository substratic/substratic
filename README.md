...
`examples/demo` is six rooms and some gems, enough to exercise the view, the
window, input, the update line and the playtest on both targets.
...
node ../../tools/serve.mjs build/web 8000
```

Open `http://127.0.0.1:8000/?playtest`. Arrows or WASD move (or the left
touch stick), Q and E turn (or the right stick), F1 shows the readout, F2
makes a report. `--selftest` (desktop) and `?selftest` (web) run the
engine's checks inside the built game on its own target.

`verify-native.sh` and `verify-web.mjs` drive the two builds headless and
